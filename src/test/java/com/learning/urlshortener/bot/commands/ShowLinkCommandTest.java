...
@SpringBootTest
class ShowLinkCommandTest extends TestContainerSupplier {
    ...
    @BeforeEach
    void setUp() {
        underTest = urlShortenerBotRegistrator.getRegisteredCommand("show_link");
    ...
    void should_report_completion_message(String languageCode, String botResponse) throws TelegramApiException {
        //given
        preparedMessage.getFrom().setLanguageCode(languageCode);
        preparedMessage.setText("/show_link");
    ...
    static Stream<Arguments> responseMessageArgumentProvider() {
        return Stream.of(
                arguments("en", "will show link details"),
                arguments("ru", "покажет детали линка"));
    }
}